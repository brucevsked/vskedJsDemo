...
function change1(v){
	var str="0";
	for(i=0;i<v;i++){
		str+=("_"+(document.getElementById(x[i]).selectedIndex-1));
	};
	var xx=document.getElementById(x[v]);
	with(xx){
		length = 0;
		options[0]=new Option(opt1[v],opt1[v]);
		if(v && document.getElementById(x[v-1]).selectedIndex>0 || !v){
			if(dsy.Exists(str)){
				ar = dsy.Items[str];
				for(i=0;i<ar.length;i++){
					options[length]=new Option(ar[i],ar[i]);
...
var opt1 = ["请选择","请选择"];//初始值
var x=["x_province","x_city"];//三个select的name
function _init_areas(){  //初始化函数
	for(i=0;i<x.length-1;i++){
	  document.getElementById(x[i]).onchange=new Function("change1("+(i+1)+")");
	}
	change1(0);
}
